...
bool CmdVarInt::Execute(const std::vector<std::string>& params)
{
	...
	const int speed = params.size() > 3 ? std::stoi(params[3]) : 1;
	const int min = params.size() > 4 ? std::stoi(params[4]) : INT32_MIN;
	const int max = params.size() > 5 ? std::stoi(params[5]) : INT32_MAX;
