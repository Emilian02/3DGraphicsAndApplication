...
#pragma pack(pop)
    ...
    X::Color GetBulinearFilterPixelColor(const Texture& tex, float u, float v)
    {
        //convert u/v coordinates to pixel x,y indexes
        float uTex = u * static_cast<float>(tex.GetWidth());
        float vTex = v * static_cast<float>(tex.GetHeight());

        //convert the float valuse to int (eg. 13.3 = 13)
        int uInt = static_cast<int>(uTex);
        int vInt = static_cast<int>(vTex);

        //get the float value from the index (eg. 13 will give 0.3)
...
X::Color Texture::GetPixel(float u, float v, AddressMode mode, bool filter)const
{
    ...
    if(filter)
    {
        return GetBulinearFilterPixelColor(*this, u, v);
    }
