...
X::Color PointLight::ComputeLightColor(const Vector3& position, const Vector3& normal)
{
    ...
    float atten = 1.0f / (mAttentConst + (mAttentLinear * distToLight) + (mAttendQuad * distToLight * distToLight));
...
{
    mAttentConst = constant;
    mAttentLinear = linear;
    mAttendQuad = quadratic;
}
